...
public:
	...
	float racket_right_x = OpenGlSystem::width - racket_width - 10;
	float racket_right_y = 50;

	// ball
	float ball_pos_x = OpenGlSystem::width / 2;
	float ball_pos_y = OpenGlSystem::height / 2;
...
public:
	...
	void updateBall() {
		...
		if (ball_pos_x < 0) {
			++score_right;
			ball_pos_x = OpenGlSystem::width / 2;
			ball_pos_y = OpenGlSystem::height / 2;
			ball_dir_x = fabs(ball_dir_x); // force it to be positive
			ball_dir_y = 0;
		}

		// hit right wall?
		if (ball_pos_x > OpenGlSystem::width) {
			++score_left;
			ball_pos_x =OpenGlSystem:: width / 2;
			ball_pos_y = OpenGlSystem::height / 2;
			ball_dir_x = -fabs(ball_dir_x); // force it to be negative
		...
		// hit top wall?
		if (ball_pos_y > OpenGlSystem::height) {
		...
		MathsHelper::vec2_norm(ball_dir_x, ball_dir_y);
	}
	//
};
